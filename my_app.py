...
if not st.session_state.password_correct:
    ...
    with tabs[0]:
        ...
        with col_l:
            ...
            H = st.number_input("厚度 (cm)", value=2.0)
            KG = st.number_input("重量 (kg)", value=0.5)
        ...
        with col_r:
            ...
            fbm_shipping = ((KG * 131 + 16) / 6.0) if "FBM" in mode else 0.0
    ...
    with tabs[2]:
        st.header("🖼️ 場景批量渲染")
        bg = st.file_uploader("上傳背景圖", accept_multiple_files=True)
        pr = st.file_uploader("上傳產品圖 (PNG)", accept_multiple_files=True)
        if st.button("執行合成") and bg and pr:
            for b in bg:
                img = Image.open(b).convert("RGBA")
                st.image(img, use_container_width=True)

    # --- 📦 模組 4: FBA 智能備貨計算器 (對齊 efed275eb 截圖) ---
    with tabs[3]:
